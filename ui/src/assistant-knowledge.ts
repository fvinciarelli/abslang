...
export const EXAMPLE_CATALOG = "- agent-decision — Agent decision — process vs escalate [agent, dataset, decision, expected, handoff, llm_judge, optional, v0.2]\n- appointment-booking — Appointment booking with fallback to human [appointment, booking, contains, handoff, multi-session, tool-calls, ui-selection, variables]\n- chatbot-greeting — Chatbot greeting [chatbot, greeting, llm_judge]\n- intent-routing — Intent routing — damaged item → refund [contains, handoff, intent, llm_judge, never, routing, sequence, tool-calls, variable_consistency, variables]\n- missing-data-multi — Missing data detection — order ID and email [clarification, data, dataset, expected, llm_judge, missing, multi, never, optional, v0.2]\n- missing-data — Missing data detection — order ID [clarification, data, dataset, expected, groundedness, llm_judge, missing, never, optional, v0.2]\n- order-status-missing-info — Order status requires order number [clarification, contains, handoff, info, missing, never, order, sequence, status, tool-calls, tool_call, variable_consistency, variables]\n- order-status — Order status [contains, order, schema, status, tool-calls, variables]\n- rag-groundedness-azure — Return policy RAG — Azure AI Foundry evaluation [azure, coherence, custom, dataset, groundedness, rag, relevance, sequence, tool-calls]\n- rag-groundedness — Return policy RAG — groundedness, relevance, coherence [coherence, dataset, groundedness, rag, relevance, sequence, tool-calls]\n- reference-metrics — Reference-based answer check [bleu, dataset, f1, metrics, reference, rouge, v0.2]\n- refund-multi-stage-parametrized — Damaged item → refund (parametrized) [contains, dataset, llm_judge, multi, parametrized, refund, relevance, sequence, stage, variable_consistency, variables]\n- refund-multi-stage — Damaged item → refund (multi-stage evaluation) [contains, llm_judge, multi, refund, relevance, sequence, stage, variable_consistency, variables]\n- refund-request — Refund request — approved [clarification, contains, handoff, llm_judge, never, refund, request, sequence, tool-calls, variable_consistency, variables]";
