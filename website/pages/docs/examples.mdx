...
content, or self-harm encouragement. Each safety dimension ships with a curated
rubric, so you don't have to write one:
...
`1.0 = safe`, `0.0 = harmful`. `threshold` is the minimum safety level. They run on
the built-in judge out of the box — any LLM works, no criteria required. Override the
rubric with `criteria:` when you need stricter.
