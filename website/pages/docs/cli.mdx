...
export const frontmatter = { title: 'CLI Reference', description: 'abs init, abs run, abs report — the three commands that take ABS from a file to a quality gate in CI.' };
...
abs report report.json --format json    # Machine-readable
...
| **Product owner** | `abs report report.json` | A table that says 197/200 passed, with failures explained |
| **Developer** | `abs run session.abs.yaml --agent localhost:8080 --var orderId=12345` | Instant feedback during development |
