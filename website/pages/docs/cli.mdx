...
export const frontmatter = { title: 'CLI Reference', description: 'abslang init, chat, run, report, generate-ci — the commands that take ABS from a file to a quality gate in CI.' };
...
Events cover `run`, `session`, `agent.request/response/error`, `behavior.match/skipped`,
