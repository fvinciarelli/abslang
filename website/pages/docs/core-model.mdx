...
The Core Model defines the conceptual building blocks of ABS. It is intentionally abstract — the formal grammar and conformance rules are in the [Specification](/abslang/docs/specification).
...
The full vocabulary with definitions is in [Vocabulary](/abslang/docs/vocabulary).

## Target

**Target** is the object or destination of the action. Its meaning depends on the action category — see the [Specification §4](/abslang/docs/specification#4-target-semantics-normative) for the normative rules.
...
Variables let you capture values at runtime and reuse them later. See [Variables](/abslang/docs/variables) for the full rules on capture, reuse, and runtime binding.
...
See [Evaluations](/abslang/docs/evaluations) for all evaluator types and semantics.
