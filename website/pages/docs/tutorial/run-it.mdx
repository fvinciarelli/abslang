...
abslang run session.abs.yaml --agent $URL --agent-format responses  # OpenAI Responses API + SSE
...
# Forward the caller's incoming Authorization header to the agent
abslang run session.abs.yaml --agent $URL --agent-forward-auth \
