...
```bash
abs run session.abs.yaml --agent $URL --agent-format openai
abs run session.abs.yaml --agent $URL --agent-format claude
abs run session.abs.yaml --agent $URL --agent-format gemini
```
