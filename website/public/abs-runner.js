// typescript/node_modules/js-yaml/dist/js-yaml.mjs
...
function expandFragments(doc) {
  const fragments = doc.fragments ?? {};
  function expand(behaviors) {
    const result = [];
    for (const entry of behaviors) {
      if ("include" in entry) {
        const fragName = entry.include;
...
function resolveVariables(behaviors, runtimeBindings = {}) {
  const variables = { ...runtimeBindings };
  return behaviors.map((b) => {
    ...
    if (b.with) {
      resolved.with = resolveObject(b.with, variables);
    }
    if (b.with_only) {
...
function resolveString(value, vars) {
...
// typescript/src/evaluators/builtin.ts
function exactMatch(observed, rule) {
  ...
  return {
    ...
    passed,
    score: passed ? 1 : 0,
    reason: passed ? `Content matches "${expected}"` : `Expected "${expected}", got "${observed}"`
  };
}
function contains(observed, rule) {
  ...
  return {
    ...
    passed,
    score: passed ? 1 : 0,
    reason: passed ? `Content contains "${rule.value}"` : `Expected content to contain "${rule.value}", got "${obs.substring(0, 100)}"`
  };
}
function regex(observed, rule) {
  const obs = String(observed ?? "");
  ...
  return {
    ...
    passed,
    score: passed ? 1 : 0,
    reason: passed ? `Content matches /${rule.pattern}/` : `Expected content to match /${rule.pattern}/, got "${obs.substring(0, 100)}"`
  };
}
function schema2(observed, rule) {
  ...
  for (const [key, schema3] of Object.entries(properties)) {
    if (key in observed) {
      const propSchema = schema3;
...
function matchesSelector(step, selector) {
  ...
  if (selector.action) {
    ...
    } else if (commActions.includes(step.action) && commActions.includes(selector.action)) {
    } else return false;
  }
  if (selector.target && step.target !== selector.target) return false;
  return true;
}
function sequence(trace, rule) {
  let traceIdx = 0;
...
function eventually(trace, rule) {
  const found = trace.some((s) => matchesSelector(s, rule.match));
  return {
    type: "eventually",
    passed: found,
    score: found ? 1 : 0,
    reason: found ? `Found matching step` : `Never found step matching ${JSON.stringify(rule.match)}`
  };
}
function never(trace, rule) {
  const found = trace.some((s) => matchesSelector(s, rule.match));
  return {
    type: "never",
    passed: !found,
    score: found ? 0 : 1,
    reason: found ? `Found disallowed step matching ${JSON.stringify(rule.match)}` : `Disallowed step never occurred`
  };
}
function count(trace, rule) {
  const n = trace.filter((s) => matchesSelector(s, rule.match)).length;
  const minOk = rule.min === void 0 || n >= rule.min;
  const maxOk = rule.max === void 0 || n <= rule.max;
  const passed = minOk && maxOk;
  return {
    type: "count",
    passed,
    score: passed ? 1 : 0,
    reason: passed ? `Count ${n} within [${rule.min ?? 0}, ${rule.max ?? "\u221E"}]` : `Count ${n} outside [${rule.min ?? 0}, ${rule.max ?? "\u221E"}]`
  };
}
function within(trace, rule) {
  let afterIdx = -1;
...
function variableConsistency(trace, behaviors, rule) {
  const values = [];
  const vars = {};
  ...
  const consistent = values.every((v) => JSON.stringify(v) === first);
  return {
    type: "variable_consistency",
    passed: consistent,
...
function evaluateComposition(trace, rule, behaviors) {
  const results = (rule.evaluations ?? []).map(
...
var adapters = {};
function registerAdapter(type2, fn) {
  adapters[type2] = fn;
}
async function evaluateWithAdapter(type2, trace, evaluation) {
  const adapter = adapters[type2];
  if (!adapter) return null;
  return adapter(trace, evaluation);
}

// typescript/src/evaluators/builtin_judge.ts
var JUDGE_SYSTEM = `You are an expert evaluator of AI assistant responses.
...
function buildPrompt(trace, criteria) {
  const traceText = trace.map(
    (s) => `[${s.actor}] ${s.action}${s.target ? " \u2192 " + s.target : ""}: ${typeof s.content === "string" ? s.content : JSON.stringify(s.content)}`
  ).join("\n");
...
async function judgeOpenAI(trace, criteria) {
...
async function judgeAnthropic(trace, criteria) {
...
async function judgeGemini(trace, criteria) {
...
function parseJudgeResponse(content, provider) {
  let score = 0.5;
...
function mockJudge(trace, evaluation) {
  ...
  return {
    ...
    passed: score >= 0.7,
    score,
    reason: `[mock] Response seems ${score >= 0.7 ? "good" : "weak"} (content length: ${lastContent.length} chars). Criteria: ${criteria.substring(0, 80)}`
  };
}
async function builtinLlmJudge(trace, evaluation) {
...
// typescript/src/runner-browser.ts
async function openaiAdapter(messages, config) {
...
async function runBrowser(yamlText, agentConfig) {
  ...
  for (const behavior of session.behaviors) {
    ...
    if (behavior.actor === "user") {
      ...
      const matched = observed ? observed.actor === behavior.actor && (observed.action === behavior.action || commActions.includes(observed.action) && commActions.includes(behavior.action) || execActions.includes(observed.action) && execActions.includes(behavior.action)) && (commActions.includes(behavior.action) || !behavior.target || observed.target === behavior.target) : false;
