...
def render_sequence_diagram(source: str, width: int = 100) -> str | None:
    ...
    for row in rows:
        ...
        if kind == "message":
            ...
            if row["a"] == row["b"]:
                for chunk in _wrap(row["label"], max(10, min(40, width - pa - 4))):
                    l = lifelines()
                    put(l, pa + 3, chunk)
                    out.append(line(l))
                arrow_line[pa] = "│"
                ...
                for chunk in _wrap(row["label"], max(10, min(40, width - pa - 3))):
                    l = lifelines()
                    put(l, pa + 2, chunk)
                    out.append(line(l))
                ...
                for chunk in _wrap(row["label"], max(10, min(40, pa - 2))):
                    l = lifelines()
                    put(l, max(1, pa - 1 - len(chunk)), chunk)
                    out.append(line(l))
        ...
        if kind == "note":
            ...
            inner = max(4, high - low - 2, min(len(row["text"]) + 2, width - low - 2))
            mid_box = lifelines()
            while len(mid_box) <= low + inner + 1:
                mid_box.append(" ")
            mid_box[low] = "│"
            mid_box[low + inner + 1] = "│"
            put(mid_box, low + 1, " " + row["text"][: max(0, inner - 2)] + " ")
